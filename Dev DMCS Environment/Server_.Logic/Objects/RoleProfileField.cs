...
namespace Web.Admin.Logic.Objects
{
    ...
    /// </summary>
    public partial class RoleProfileField : ApplicationObject, IApplicationObject
    {
    ...
    public partial class RoleProfileField : ApplicationObject, IApplicationObject
    {
        ...
        [DataMember]
        public int RoleFieldID { get; set; }
        ...
        public RoleProfileField(IDataReader dr)
        {
            this.Load(dr);
        }
        ...
        public virtual bool Load(IDataReader dr)
        {
            ...
            this.RoleFieldID = DBUtil.GetIntField(dr, "RoleFieldID");
        ...
        public virtual bool Save()
        {
                ...
                case DatabaseProvider.MSSQLServer:
                    {
                        ...
                        IF NOT EXISTS(SELECT ProfileFieldID FROM {0} WHERE ProfileFieldID = @ProfileFieldID  AND RoleID = @RoleID)
                           BEGIN
                            INSERT INTO {0} (RoleID, ProfileFieldID) 
                                VALUES(@RoleID, @ProfileFieldID); SELECT @RoleFieldID = @@IDENTITY
                        END
                        ELSE
                            SELECT @RoleFieldID = RoleFieldID FROM {0} WHERE ProfileFieldID = @ProfileFieldID  AND RoleID = @RoleID
                        ", Table));
                        break;
                    }
                case DatabaseProvider.MySQLServer:
                    {
                        sp = new SpRoleAdmin(String.Format(@"
                            INSERT INTO {0} (RoleID, ProfileFieldID) 
                                      SELECT @RoleID, @ProfileFieldID FROM DUAL
                                            WHERE NOT EXISTS(SELECT * FROM {0} WHERE ProfileFieldID = @ProfileFieldID  AND RoleID = @RoleID) LIMIT 1;
            ...
            sp.AddParameter("RoleFieldID", RoleFieldID, System.Data.ParameterDirection.Output);
            ...
            if (this.RoleFieldID <= 0)
            {
                ...
                switch (DbRoleAdmin.CurrentProvider)
                {
                    case DatabaseProvider.MSSQLServer:
                        {
                            RoleFieldID = (int)sp.GetParameter("RoleFieldID");
                            break;
                        }
                    case DatabaseProvider.MySQLServer:
                        {
                            ...
                            if (cm.LastInsertedId > 0)
                                RoleFieldID = Convert.ToInt32(cm.LastInsertedId);
        ...
        public virtual bool Delete()
        {
            ...
            SpRoleAdmin sp = new SpRoleAdmin(String.Format("DELETE FROM {0}  WHERE RoleID = @RoleID AND ProfileFieldID = @ProfileFieldID", Table));
            sp.AddParameter("RoleID", RoleID);
            sp.AddParameter("ProfileFieldID", ProfileFieldID);
